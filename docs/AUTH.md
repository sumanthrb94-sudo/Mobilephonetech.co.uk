...
**The email address, and only the email address.** Every account is one
Firebase Auth user keyed by `uid`, and the profile document lives at
`users/{uid}`.

Phone numbers are **not** an identity. A number captured at checkout is a
delivery contact stored on the order (`shippingAddress.phone`) — it is never
verified, never unique, and two accounts may carry the same one. There is no
phone sign-in.
...
**Phone sign-in.** It needs reCAPTCHA, costs money per SMS, and brings its own
duplicate problem — a number that changes hands, or a customer with two
numbers. Email remains the primary identity for a UK refurbished-phone shop.
The groundwork for delivery SMS exists in `api/_sms.ts`, but that is a
notification channel, not a login.
...
3. **Email/Password and Google are both enabled** under Sign-in method.
